...
### 2. Data Parts  
Data parts contain structured data for rich interactive components like weather widgets, file attachments, sources, and more. They use types with the `data-` prefix and can include any custom data structure.
...
interface DataPart {
  ...
  type: string // should use 'data-' prefix for custom parts
  data?: any
}
```
...
3. Each component uses `usePartData(partType)` to check if it should render
...
### Suggestion Parts (`data-suggested-questions`)
...
Create domain-specific parts for specialized content by implementing a custom component:

### 1. Define the Part Type and Data Interface

```typescript
// Use 'data-' prefix for custom parts
...
export function WeatherPart() {
  // usePartData returns data only if current part matches the type
...
Parts are streamed using the **Server-Sent Events (SSE)** protocol, which provides real-time communication between the server and client. Here's how the streaming implementation works:
...
function CustomPartComponent() {
  ...
  const weatherData = usePartData<WeatherData>('data-weather')
  const textContent = usePartData<string>('text')
