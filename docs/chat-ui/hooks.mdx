...
Access the current message part context within part components. This hook provides type-safe access to specific part types.
...
function CustomPartComponent() {
  const customPart = usePart<CustomPartType>('custom-type')
...
- `usePart('file')` → `FilePart | null`
- `usePart('data-artifact')` → `ArtifactPart | null`
- `usePart('event')` → `EventPart | null`
- `usePart('sources')` → `SourcesPart | null`
- `usePart('suggestion')` → `SuggestionPart | null`
