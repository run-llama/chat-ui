...
A specialized hook for building chat interfaces with LlamaDeploy workflows. It provides a `ChatHandler` that works seamlessly with chat-ui components.
...
- `deployment` - LlamaDeploy deployment name
- `workflow` - Workflow service name (optional)
- `baseUrl` - API server URL (optional)
- `onError` - Error callback

**Key Features:**

- **Chat Handler**: Returns a `ChatHandler` compatible with all chat-ui components
- **Auto Processing**: Converts workflow events into chat messages and annotations
- **Event Support**: Handles `SourceNodesEvent`, `ArtifactEvent`, and `UIEvent` from workflows

The hook automatically processes workflow events and renders them as annotations in the chat interface, making it easy to build rich conversational experiences with LlamaDeploy workflows.
