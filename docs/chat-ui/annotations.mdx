...
const stream = new ReadableStream({
  start(controller) {
    ...
    controller.enqueue(
      encoder.encode(`8:${JSON.stringify([annotation])}\\n`)
    )
...
<ChatMessage message={message}>
  <ChatMessage.Content>
    ...
    <ChatMessage.Content.Image />  {/* Automatically renders IMAGE annotations */}
  </ChatMessage.Content>
</ChatMessage>
```
...
function WeatherIcon({ condition }: { condition: string }) {
  ...
  return (
    <span className="text-2xl">
      {iconMap[condition.toLowerCase()] || '🌤️'}
    </span>
  )
}
```
...
function useWeatherData() {
  ...
  return getCustomAnnotations<WeatherData>(
    message.annotations,
    'weather'
  )
}
...
Validate annotation data before rendering:
...
function SafeWeatherWidget() {
  const { message } = useChatMessage()
  const weatherData = getCustomAnnotations(message.annotations, 'weather')
  
  if (!weatherData[0] || !validateWeatherData(weatherData[0])) {
    return null
  }
  
  return <WeatherDisplay data={weatherData[0]} />
}
```
...
- **Lazy Load Images** - Use loading="lazy" for image annotations
- **Optimize File Sizes** - Compress images and limit file upload sizes
- **Cache Annotations** - Cache processed annotation data when possible

### Accessibility

- **Alt Text** - Always provide alt text for images
- **Keyboard Navigation** - Ensure interactive annotations are keyboard accessible
- **Screen Reader Support** - Use semantic HTML and ARIA labels

### User Experience

- **Loading States** - Show loading indicators for annotation processing
- **Error Handling** - Gracefully handle failed annotation rendering
- **Progressive Enhancement** - Ensure content works without annotations
