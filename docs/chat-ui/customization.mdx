...
function PDFArtifactViewer() {
  ...
  if (displayedArtifact?.type !== 'pdf') return null
  
  const { data } = displayedArtifact as Artifact<{ pdfUrl: string }, 'pdf'>
...
// Custom chart viewer
function ChartArtifactViewer() {
  const { displayedArtifact } = useChatCanvas()
  
  if (displayedArtifact?.type !== 'chart') return null
  
  const { data } = displayedArtifact as Artifact<{
    chartData: any[]
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex items-center justify-between border-b p-4">
        <h3 className="flex items-center gap-3">
          <BarChart className="size-8 text-green-500" />
      ...
      <div className="flex-1 p-4">
        <CustomChart data={data.chartData} type={data.chartType} />
      </div>
    </div>
  )
}

// Include all viewers in your canvas
